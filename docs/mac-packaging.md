...
窗口使用普通 NSWindow 层级以接收鼠标，保留无边框、不置顶和跨桌面显示。原生属性在 `before_show` 主线程事件中设置，不再从后台启动回调设置桌面层级。左侧标题及四周 12px 边缘（含四角）使用鼠标位移调整原生窗口位置，避免多屏环境重复计算屏幕原点；Mac 禁用原生边缘缩放，避免与拖动竞争。拖动超过 3px 后开始移动，按动画帧合并位移并串行调用原生接口，松手会保留末段位移；取消、失焦或桥接失败会释放鼠标捕获。按钮区不参与拖动。设置按钮打开弹窗，看板通过 Python 接口打开默认浏览器，× 仅隐藏窗口，接口未就绪或调用失败会显示提示。
