...
.venv/bin/python -m unittest discover -s tests -p test_mac_packaging.py -v
...
需要 Apple Developer Program 的 **Developer ID Application** 证书及其私钥，不能用临时或自签名证书替代。在仓库 Actions Secrets 配置：
...
发布 `vX.Y.Z` Release 时，CI 分别在 `macos-15`（arm64）和 `macos-15-intel`（x86_64）构建。依次执行嵌套代码签名、最终 App 签名、DMG 签名、Apple 公证、附加公证票据及镜像校验。缺失凭据、公证未获 Accepted 或任何校验失败，均不上传该架构的 Release 附件。分支 push 和手动 workflow_dispatch 仅产生测试 Artifact，不上传正式 Release，也不使用发布私钥。
...
构建会验证 App 及 DMG 内 App 的完整签名，在去除 `PYTHONHOME`、`PYTHONPATH` 且仅保留系统 PATH 的环境中执行内置运行环境自检，确认 Cocoa 依赖和前端资源可加载。正式发布额外验证公证票据、`notarized` 代码要求及 Gatekeeper。自检不连接 Supabase、不读取真实账号配置、不打开 GUI。
